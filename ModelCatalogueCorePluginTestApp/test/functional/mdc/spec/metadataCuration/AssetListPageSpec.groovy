...
class AssetListPageSpec extends GebReportingSpec {
	...
	def "Asset list page shows a list of assets and Status filter button"() {
		...
		waitFor {
			...
			$(assetList).find("tbody tr td a",0).displayed
		}
		waitFor {
			//asset name should be "DraftDefaultLayout"
			$(assetList).find("tbody tr td a",0).text() == "defaultLayout"
		}
	}
	...
	def "Selecting Draft status filter will just show the Draft assets"(){
		...
		//the list of status filter (Draft,Finalized,..) will be displayed
		waitFor {
		...
		waitFor {
			//asset name should be "DraftDefaultLayout"
			$(assetList).find("tbody tr td a",0).text() == "DraftDefaultLayout"
		}
	}
